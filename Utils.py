...
class Utils:
    ...
    def get_angle_limited_uv(self,vector_to_limit, vector_base_line, angle_limit_degs):
        angle_between = Mat.Mat().angle_between_degrees(vector_base_line, vector_to_limit)
        if angle_between > angle_limit_degs:
            correction_axis = Mat.Mat().normalization(
                np.cross(Mat.Mat().normalization(vector_base_line), Mat.Mat().normalization(vector_to_limit)))
            return Mat.Mat().normalization(Mat.Mat().rotate_about_axis(vector_base_line, angle_limit_degs, correction_axis))
        else:
    ...
    def get_signed_angle_between_degs(self,reference_vector, other_vector, normal_vector):
        unsigned_angle = Mat.Mat().angle_between_degrees(reference_vector, other_vector)
        sign_measure = Mat.Mat().dot_product(np.cross(reference_vector, other_vector), normal_vector)
        if sign_measure >= 0:
            sign = 1
        else:
            sign = -1
    ...
    def approximately_equal(self,a, b, tolerance):
